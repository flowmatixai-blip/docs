---
title: "Integrate  Flowmatix with Pabbly"
description: "This guide provides step-by-step instructions on how to integrate Pabbly with  Flowmatix to collect leads captured on  Flowmatix and transfer them to a third-party application, such as Google Sheets."
---
...
A trigger is an event that starts an automation. For example, when a new lead is captured on  Flowmatix, that event can trigger an action in another app. Triggers tell Pabbly when to start a workflow.
...
### **How Pabbly Helps with  Flowmatix**

Integrating Pabbly with  Flowmatix allows businesses to automate workflows and enhance chatbot efficiency. When a **new contact is created**, a **message is received in chat**, or a **WhatsApp order is placed**, these triggers can initiate automated actions in other applications. Similarly, when triggered by a third-party app through Pabbly,  Flowmatix can **send WhatsApp templates, update custom attributes, send session messages, or add new contacts**. This seamless automation reduces manual work, improves response time, and ensures smooth communication across platforms.

### **Prerequisites**

The following prerequisites need to be met when you are integrating Pabbly with  Flowmatix.

- An active Pabbly account with available **task credits** for automation processes.

---

### **Steps to Integrate Pabbly with  Flowmatix**

#### **Section A: Setting up Integration in  Flowmatix**

1. Login to your  Flowmatix account and select the chatbot you want to integrate with Pabbly. Click on the “**Third-party Apps Integration”.**
2. \
...
7. \
   ...
   - **New Lead**: Trigger which occurs whenever a new contact is created in  Flowmatix.
...
#### **Section B: Setting up  Flowmatix Trigger on Pabbly:**
...
3. \
   Upon clicking “**Create**”, automatic redirection should occur to the workflow as shown below. For creation of trigger from  Flowmatix, search “** Flowmatix**” in the “**Search App**”.
4. \
   After searching and selecting  Flowmatix, select the same type of “**Trigger Event**” you want from the dropdown menu as in  Flowmatix. These can include:
   - **New Lead**: Trigger which occurs whenever a new contact is created in  Flowmatix.
   - **New Incoming Messages**: Occurs when a new message is incoming in the selected chatflow being integrated.
   - **New WhatsApp Order Created**: When a new order is placed.
5. \
   Upon selection, the box will expand as seen below, including in itself several more details. Copy the “**Webhook URL**” and paste it in the “**Pabbly Webhook URL**” in  Flowmatix Dashboard.
6. \
   Paste the copied Webhook URL in the “**Pabbly Webhook URL**” on  Flowmatix, and then click “**Verify & Add Webhook URL**”.
...
And the trigger setting has been completed\! Configure the corresponding action using the “Action box” on Pabbly(below Trigger box), and enjoy workflow benefits of  Flowmatix with other applications.

#### **Section C: Setting up  Flowmatix Action on Pabbly:**
...
3. \
   ...
   <Info>
     Have a working Trigger before proceeding. Below steps only detail the set-up of  Flowmatix Action, not the creation of Trigger for any third parties.
   </Info>
4. \
   Search for “** Flowmatix**” in the “**Search App**” area, and select the same.
5. Select the action that you wish to occur. Options include:
   - **Add Contact** – Add a new contact to your  Flowmatix account.
...
6. \
   ...
   - Add New Connection: The connection that will be established with  Flowmatix
   - Select Existing Connection: Selecting an already existing connection for the purposes of communication
7. Enter the API Token, that in the developer section of your  Flowmatix Account, and add the same to the “**Token**” section and click “**Save**”.
8. \
   Post successful save, information will be need to be entered or mapped that will be sent to  Flowmatix. Edit the same in the given columns.
9. \
